...
function Vinci({ cartItems = [], setCartItems }) {
  ...
  const cards4Data =
    Array.isArray(data)
      ? data.find((item) => item.cards4)?.cards4 || []
      : [];
  const isInCart = (item) => {
  ...
  return (
    <>
      ...
      >
        Aromatic Bath & Body, Rooted in Rare Botanicals
      </Typography>
      <Box
        sx={{
          width: "100%",
      ...
      >
        ...
        >
          {cards4Data.slice(0, 9).map((item, index) => (
            <Box
              key={index}
              sx={{
