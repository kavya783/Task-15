...
function NewLaunches({ cartItems = [], setCartItems }) {
  ...
  const cards3Data =
    Array.isArray(data)
      ? data.find((item) => item.cards3)?.cards3 || []
      : [];
  //       useEffect(() => {
  ...
  return (
    ...
    <>
      ...
      >
        New formulas to love every day
      </Typography>
      <Box
        sx={{
          width: "100%",
      ...
      >
        ...
        >
          {cards3Data.slice(0, 9).map((item, index) => (
            <Box
              key={index}
              sx={{
