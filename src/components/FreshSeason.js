...
function FreshSeason({ cartItems = [], setCartItems })  {
...
const [snackOpen, setSnackOpen] = useState(false);
  ...
  const cards2Data =
    Array.isArray(data)
      ? data.find((item) => item.cards2)?.cards2 || []
      : [];
       const isInCart = (item) => {
    ...
    <>
      <Typography sx={{
        ...
        fontSize: Theme.font18Bold,
          color:Colors.gray
      }}>Fresh Season Essentials</Typography>
      <Typography
        sx={{
          ml: { xs: 2, md: 15 },
          mb: 3,
      ...
      >
        Lightweight care for the season ahead
      </Typography>
      <Box
        sx={{
          width: "100%",
      ...
      mr: { xs: 3, md: 15 },
        ...
        >
          {cards2Data.slice(0, 9).map((item, index) => (
            <Box
              key={index}
              sx={{
                minWidth: {
  ...
  >
    {item.description}
              ...
              >
                ...
                <Typography sx={{ fontSize: Theme.font14Bold, ml: 5 }}>{item.c}</Typography>
              </Box>
